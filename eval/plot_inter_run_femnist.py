...
export_as_pdf = True
base_dir = Path("data/experiments/femnist/degree_3")
...
def plot_stability(stability_data, output_path, export_as_pdf):
    ...
    ax.set_xlabel("Communication Rounds", fontsize=font_size, weight='bold')
...
def plot_combined_metrics(metric_data, stability_data, bytes_data, output_path, time_mappings, plot_over_time, export_as_pdf):
    fig, axes = plt.subplots(2, 2, figsize=(20, 12))  # 2x2 layout
    ylabels = {
        "test_acc": "Accuracy",
        "test_loss": "Loss",
        "stability": "Variance",
        "total_bytes": "Bytes Sent"
    }
    font_size = 24
    metrics = ["test_acc", "test_loss", "stability", "total_bytes"]
    ...
    for idx, (metric, ax) in enumerate(zip(metrics, axes.flat)):
        if metric == "stability":
            ...
            data_dict = metric_data[metric] if metric != "total_bytes" else bytes_data
            for label, (mean_dict, std_dict) in data_dict.items():
                config = style_map.get(label, {"label": label, "color": "gray", "linestyle": "-"})
                label_clean = config["label"]
                color = config["color"]
                linestyle = config["linestyle"]
...
def main():
    ...
    plot_combined_metrics(all_metric_data, stability_data, total_bytes_data, output_path, time_mappings, plot_over_time, export_as_pdf)
