...
def analyze_folder(base_path_str):
    ...
    for algo in ["fully_connected", "diss_dl", "epidemic"]:
        ...
        acc_round = find_round_to_reach_target(metrics_summary[algo]["avg_acc_per_round"], top_accuracy_epidemic, mode='max')
        loss_round = find_round_to_reach_target(metrics_summary[algo]["avg_loss_per_round"], top_loss_epidemic, mode='min')
    ...
    output = {
        "top_accuracy_and_loss": {
            algo: {
                "top_accuracy": metrics_summary[algo]["top_accuracy"],
                "top_loss": metrics_summary[algo]["top_loss"]
            }
...
analyze_folder("data/experiments/cifar/degree_3")
