...
def wait_for_files(prefix, count, timeout=120):
    ...
    while True:
        ...
        if time.time() - start > timeout:
            raise TimeoutError(f"Timeout waiting for {prefix} files")
        time.sleep(0.5)


def save_model(uid, model):
    os.makedirs(TMP_DIR, exist_ok=True)
    torch.save(model.state_dict(), f"{TMP_DIR}/weights_{uid}.pt")


def write_done(uid):
    open(f"{TMP_DIR}/done_{uid}", "w").close()


def load_all_models(model_template):
    weights = {}
    for f in glob.glob(f"{TMP_DIR}/weights_*.pt"):
        uid = int(os.path.basename(f).split("_")[1].split(".")[0])
        m = model_template()
...
def compute_global_model(all_models):
    ...
    for k in state_dicts[0]:
        global_state[k] = sum(sd[k] for sd in state_dicts) / len(state_dicts)

    model_class = type(next(iter(all_models.values())))
    model = model_class()
...
def cosine_similarity_per_layer(model1, model2):
    similarities = []
    with torch.no_grad():
        for name, p1 in model1.state_dict().items():
            ...
            if norm1 > 0 and norm2 > 0:
                sim = F.cosine_similarity(p1 / norm1, p2 / norm2, dim=0).item()
...
def compute_similarity_to_global(uid, model_template):
    ...
    """
    all_models = load_all_models(model_template)
    if uid not in all_models:
        raise ValueError(f"Model for uid {uid} not found.")

    global_model = compute_global_model(all_models)
